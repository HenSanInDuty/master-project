        ...
        if st.button("Bắt đầu huấn luyện"):
            with st.spinner("Đang huấn luyện"):
                try:
                    ...
                    # Hiển thị kết quả
                    st.write("Kết quả của mô hình, có thể thu gọn hoặc mở ra để xem chi tiết")
                    json_string = json.dumps(result_trainning)
...
'word_similarity':{'type': 'loại công cụ cho độ tương tự từ', 
        ...
        if st.button("Bắt đầu tóm tắt") or st.session_state.btn_result:
            ...
            with st.spinner("Đang tiến hành tóm tắt văn bản"):
                ...
                # -- Tab thông tin độ đo
                with tab4:
                    metric = pd.DataFrame.from_dict(result["metric_info"], orient='index', columns=['Result'])
                    st.dataframe(metric)
